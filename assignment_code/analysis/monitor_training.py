...
def print_summary(data, steps_per_epoch, total_epochs):
    """终端打印当前训练摘要。"""
    M = steps_per_epoch
    print(); print('=' * 60)
    ...
    if tr:
        s, v = tr[-1]
        ep = s / M
        pct = min(100, 100 * ep / total_epochs)
        bar = '█' * int(pct / 5) + '░' * (20 - int(pct / 5))
        print(f'  Epoch: {ep:.1f} / {total_epochs}  [{bar}] {pct:.1f}%')
...
def main():
    ...
    def run_once():
        ...
        print_summary(data, M, args.total_epochs)
        return data
